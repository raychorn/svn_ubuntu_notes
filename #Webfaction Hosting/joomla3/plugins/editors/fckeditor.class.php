...
class plgEditorFckeditor extends JPlugin {
	...
	function onDisplay( $name, $content, $width, $height, $col, $row, $buttons = true )
	{
		...
		$wwidth				=   $this->params->def( 'wwidth', '750' );
      ...
      /* Generate the Output */
		...
		$doc->addScriptDeclaration(
			...
			oFCKeditor.Config["ForceInlineStyles"] = "'. $this->inline.'";	
		...
		if( $showerrors && ( $my->usertype == 'Super Administrator' || $my->usertype == 'Administrator' ) ){
			...
			if( function_exists( "curl_init" ) ){
				...
				$version = curl_exec($ch); curl_close( $ch ); if( $version != '2.6.3' && $version ){ $errors .= 'Please beaware there is a newer version of the JoomlaFCK Editor which can be downloaded from <a href="http://www.joomlafckeditor.com" target="_blank">http://www.joomlafckeditor.com</a>.<br/>'; }//end if
			}//end if
