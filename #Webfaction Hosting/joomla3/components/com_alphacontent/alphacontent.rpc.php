...
$joomlaversion15 = "0";
$path = str_replace("//components/com_alphacontent/alphacontent.rpc.php", "", getenv("SCRIPT_FILENAME") );
$path = str_replace("/components/com_alphacontent/alphacontent.rpc.php", "", getenv("SCRIPT_FILENAME") );
...
if ( !$joomlaversion15 ) {
	...
	// these are new queries to get the new values!
	$newtotals = mysql_query("SELECT total_votes, total_value, used_ips, component FROM ".$dbprefix."alphacontent_rating WHERE id='".$id_sent."' AND component='".$component."'")or die(" Error: ".mysql_error());
...
$new_back[] .= '</ul></div>';
$new_back[] .= '<p class="voted">'._AJAX_ALPHACONTENT_RATING.' <strong>'.@number_format($sum/$added,1).'</strong>/'.$units.' ('.$count.' '.$tense.') ';
$new_back[] .= '<span class="thanks">'._AJAX_ALPHACONTENT_THANKS.'</span></p>';
