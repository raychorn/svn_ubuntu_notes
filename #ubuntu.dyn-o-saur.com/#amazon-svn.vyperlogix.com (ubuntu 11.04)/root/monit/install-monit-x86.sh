#!/bin/bash
cd /opt
if [ -d /opt/monit-5.3-linux-x86 ]; then
	echo "/opt/monit-5.3-linux-x86 already exists"
	p=$(monit -V | grep 5.3)
	if [ "$p" = "5.3" ]; then
		...
		ln -s /opt/monit-5.3-linux-x86/monit-5.3/bin/monit /usr/sbin/monit
	fi
else
	wget http://downloads.vyperlogix.com/Installer/monit/monit-5.3-linux-x86.tar.gz
	mkdir /opt/monit-5.3-linux-x86
	tar -xvzf monit-5.3-linux-x86.tar.gz -C /opt/monit-5.3-linux-x86
	if [ -d /opt/monit-5.3-linux-x86 ]; then
		rm -f /opt/monit-5.3-linux-x86.tar.gz
	fi
